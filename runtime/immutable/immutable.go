// This is a fork of https://github.com/benbjohnson/immutable/
// MIT License Copyright 2019 Ben Johnson

// Changes:
//   - Map.set looks for a Hasher at runtime, using the `Hash` trait implementation based on the key type.
...
// -----------------------------------
type HashMakeFn = func(key any) Hasher

var defaultHashFn HashMakeFn = nil

func SetDefaultHasher(fn HashMakeFn) {
	defaultHashFn = fn
}
...
func (m *Map) set(key, value interface{}, mutable bool) *Map {
	...
	hasher := m.hasher
	if hasher == nil {
		if defaultHashFn == nil {
			panic("Default Hashing function not set")
		}

		// hasher = NewHasher(key)
		hasher = defaultHashFn(key)
	}
